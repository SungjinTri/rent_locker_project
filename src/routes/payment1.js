...
function QRCodePayment() {
    ...
    useEffect(() => {
        const fetchPaymentStatus = async () => {
            try {
                const paymentDoc = await getDoc(doc(db, "locker", "locker 1"));
                ...
                if (paymentDoc.exists()) {
                    ...
                    if (paymentStatus === 'paid') {
                        ...
                        if (user) {
                            ...
                            await setDoc(doc(db, "locker", "locker 1"), { 
                                service: 'unavailable', 
                                customer: uid, 
                                pin: generatedPin 
                            }, { merge: true });
    ...
    const handleConfirm = () => {
        window.location.href = '/control1'; // ไปที่หน้า home
    };

    return (
        <div className="qr-container">
            <h1>QR Code สำหรับการชำระเงิน</h1>
            <img src="/QR/qr1.jpg" alt="QR Code" className="qr-image" />
